...
from agents.prompts import resolve_prompt_path
from backend.core.models import LiveIntentResponse
...
DEFAULT_PROMPT_PATH = ROOT_DIR / "agents" / "prompts" / "live_intent" / "system.txt"
DEFAULT_PROMPT_TEXT = """
...
class LiveIntentClassifier:
    def __init__(
        ...
        *,
        ...
        if raw_prompt_path:
            try:
                self.prompt_path = resolve_prompt_path(raw_prompt_path)
            except FileNotFoundError:
                candidate = Path(raw_prompt_path)
                self.prompt_path = (
                    candidate
